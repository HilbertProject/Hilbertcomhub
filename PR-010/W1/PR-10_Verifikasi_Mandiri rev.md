...
**Ringkasan hasil:** dari **48** klaim faktual disisir (ledger utama, bernomor #1–48 lengkap), **4 terbukti keliru dan sudah diperbaiki langsung di Draft MVC** (bukan cuma dicatat di ledger — lihat aturan `format-verifikasi-mandiri.md` §"Kalau klaim gagal verifikasi"), **6 butuh catatan presisi tambahan** (✅ Akurat, tapi...), sisanya (21 Terverifikasi + 3 Terverifikasi-ulang + 7 Fakta mapan + 1 Fakta-mapan-rentang-luas + 1 Diklaim + 4 Reuse Registry di ledger utama, plus 3 lagi Reuse Registry di Lampiran) terverifikasi, fakta mapan, atau cross-check registry.

> **Catatan koreksi (dari jendela Audit Konsistensi Eksternal — `PR-010_W1_Audit-Konsistensi-Eksternal.md`):** ringkasan hasil di atas sebelumnya salah hitung dua kali — ditulis "sekitar 55 klaim" (aktual: **48**, dihitung ulang programatik dari nomor ledger) dan "2 butuh catatan presisi" (aktual: **6**, kadang dilaporkan 7 jika baris berlabel non-baku "✅ Akurat" tanpa "tapi..." turut dihitung). Ironi yang layak diakui terbuka: dokumen yang isinya tentang verifikasi klaim orang lain ternyata tidak menghitung ulang klaim ringkasannya sendiri secara terprogram sebelum dipublikasikan — persis jenis kelalaian yang coba dicegah ledger ini pada Draft MVC. Kedua angka sudah dikoreksi di atas, dihitung ulang via skrip (bukan manual) untuk menghindari pengulangan kesalahan yang sama.
